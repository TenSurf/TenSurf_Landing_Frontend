      ...
      <div className={`col-span-12 lg:col-span-5 relative h-full w-full ${direction == "left" ? "lg:order-1" : "lg:order-2"}`}>
        <div className={"about-us-text lg:absolute lg:bottom-0 lg:left-0 w-full h-fit flex flex-col gap-8"}>
          <div
            ...
            className="z-0 absolute bottom-16 left-20 w-[350px] h-[200px] rounded-full"
          ></div>
          <h2 className={"text-3xl font-bold !text-[#E9ECEF]"}>{title}</h2>
          <p className={"text-xl font-normal !text-[#CED4DA]"}>
            {description}
          </p>
        </div>
      </div>
      <div
        className={`col-span-12 lg:col-span-7 lg:max-w-[700px] mx-auto relative pb-[90vh] w-full rounded-3xl overflow-hidden item-image ${direction == "left" ? "lg:order-2" : "lg:order-1"}`}>
