...
export const HeroSection: FC<IProps> = (
  ...
  const [videoLoading, setVideoLoading] = useState<"idl" | "pend" | "full">("idl");
  ...
  return (
    ...
    >
      <div
        ...
        <div
          className={"relative rounded-2xl overflow-hidden h-fit mx-auto sm:max-w-2xl lg:max-w-none w-full"}>
          <div
            className={`${videoLoading == "full" && "hidden"} cursor-pointer bg-black z-[11] absolute w-full h-full opacity-30 backdrop-blur flex items-center justify-center`}>
            {videoLoading == "pend" ? (
              <div role="status">
              ...
              <>
                ...
                }} size={64} className={"stroke-[#8f8f8f] hover:stroke-[#ffffff]"}
                      strokeWidth={6} />
              </>
            )}

          </div>
          <video ref={videoRef} poster={"/images/video-preview.png"} muted loop playsInline width={"100%"}
                 height={"100%"}>
            <source src={`${process.env.NEXT_PUBLIC_HUB_URL}/videos/tensurf-demo.mp4`} type="video/mp4" />
          </video>
        </div>
      </div>
