		...
		>
			<div className="flex items-center flex-col justify-center gap-14 relative z-[9] max-w-3xl min-h-screen">
				<div className="flex items-center flex-col justify-center gap-6">
					<h1
						className={
							'text-5xl sm:text-7xl text-center font-medium text-[#F8F9FA] !leading-[5.5rem]'
						}
					>
						{title}
					</h1>
					<p
						className={
							'text-lg sm:text-xl font-normal text-center text-[#DEE2E6]'
						}
					>
						{description}
					</p>
				...
				<Link
					...
					href={
						cookieStore.get(token_name)
							? process.env.NEXT_PUBLIC_HUB_URL ?? ''
							: '/signin'
					}
					target={cookieStore.get(token_name) ? '_blank' : '_self'}
				>
					<Button
						className="flex gap-1 sm:gap-2 rounded-3xl drop-shadow-[3px_5px_24px_#082FDF] py-4 px-8 sm:py-8 sm:px-16">
						<p
							className={
								'text-lg sm:text-xl font-normal sm:font-bold text-center'
							}
						>
							<span className={'hidden sm:inline-block'}>Go to </span>
							<span>TenSurf Hub</span>
						</p>
			...
			<div
				className="absolute z-0 h-full top-0 w-full items-stretch justify-between flex gap-[200px] max-w-[90%] max-h-screen px-4 overflow-hidden">
